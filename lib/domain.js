...
export function metadataRepository(metadata) {
  ...
  return parseGitHubRepository(metadata.repository)
}
...
export function buildPluginCatalog(snapshot, metadataByModule = {}) {
  ...
  return sourceRows.filter((row) => {
    ...
    return {
      ...
      reason: repository ? undefined : 'No public GitHub repository metadata was found.',
    }
  })
}
...
export function findDuplicateIssues(issues, draft, limit = 5) {
  ...
  return (Array.isArray(issues) ? issues : []).map((issue) => {
    ...
    return { number: issue.number, title: text(issue.title, 240), url: text(issue.html_url || issue.url, 1000), score }
