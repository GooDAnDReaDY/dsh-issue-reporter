...
const REDACTIONS = [
  { kind: 'token', pattern: /\b(?:gh[pousr]_[A-Za-z0-9_]{20,}|github_pat_[A-Za-z0-9_]{20,}|sk-[A-Za-z0-9_-]{20,}|xox[baprs]-[A-Za-z0-9-]{12,})\b/g, replacement: '[redacted token]' },
  { kind: 'credential', pattern: /\b(?:bearer|token|password|passwd|secret|api[-_ ]?key)\s*[:=]\s*[^\s,;]+/gi, replacement: '[redacted credential]' },
...
export function composeIssueDraft(input = {}) {
  ...
  const envParts = []
  if (input.environment) envParts.push(input.environment)
  if (input.diagnostics) envParts.push('Diagnostics:\n```text\n' + input.diagnostics.trim() + '\n```')
...
export function buildAiOptimizationPrompt({ title, observed, reproduction, expected, pluginName, errorStack, diagnostics } = {}) {
  const parts = [
    ...
    '',
    `Target Plugin: ${pluginName || 'Unknown'}`,
    `Raw Title: ${title || ''}`,
    `Raw Observed: ${observed || ''}`,
    `Raw Reproduction: ${reproduction || ''}`,
    `Raw Expected: ${expected || ''}`,
  ]
  if (errorStack) parts.push(`Error Stack:\n\`\`\`\n${errorStack}\n\`\`\``)
  if (diagnostics) parts.push(`Environment Diagnostics:\n\`\`\`\n${diagnostics}\n\`\`\``)
