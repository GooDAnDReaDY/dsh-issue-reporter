...
const zh = {
    ...
    function SettingsCard({ ctx }) {
      ...
      const [configDraft, setConfigDraft] = React.useState({ appClientId: '', tokenEnv: 'GITHUB_ISSUE_REPORTER_TOKEN', apiBaseUrl: 'https://api.github.com' })
      ...
      const load = React.useCallback(async () => {
        ...
        try {
          ...
          setPlugins(merged)
          setConfigDraft({
            appClientId: result.config?.appClientId || '',
            tokenEnv: result.config?.tokenEnv || 'GITHUB_ISSUE_REPORTER_TOKEN',
            apiBaseUrl: result.config?.apiBaseUrl || 'https://api.github.com',
          })
      ...
      const saveSettings = () => {
        if (!scope) return
        scope.set('appClientId', configDraft.appClientId.trim())
        scope.set('tokenEnv', configDraft.tokenEnv.trim() || 'GITHUB_ISSUE_REPORTER_TOKEN')
        scope.set('apiBaseUrl', configDraft.apiBaseUrl.trim() || 'https://api.github.com')
        setStatus((current) => current ? { ...current, config: { ...current.config, appClientId: configDraft.appClientId, tokenEnv: configDraft.tokenEnv, apiBaseUrl: configDraft.apiBaseUrl } } : current)
      }
      ...
      const body = !open
        ? h('div', null, [
            h('p', { style: { margin: '6px 0', color: 'var(--dsw-alias-label-secondary)' } }, t.description),
            h('button', { type: 'button', onClick: () => setOpen(true) }, t.report),
          ])
        : h('div', null, [
            h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' } }, [
              h('button', { type: 'button', onClick: load, disabled: busy }, t.refresh),
              status?.config?.tokenConfigured ? h('span', null, '* ' + t.connected) : h('span', null, t.authRequired),
            ]),
            h('section', { style: { marginTop: 12, paddingTop: 8, borderTop: '1px solid var(--dsw-alias-border-l1)' } }, [
              h('strong', null, t.auth),
              h(Field, { label: t.appClientId, value: configDraft.appClientId, onChange: (value) => setConfigDraft({ ...configDraft, appClientId: value }) }),
              h(Field, { label: t.tokenEnv, value: configDraft.tokenEnv, onChange: (value) => setConfigDraft({ ...configDraft, tokenEnv: value }) }),
              h(Field, { label: t.apiBaseUrl, value: configDraft.apiBaseUrl, onChange: (value) => setConfigDraft({ ...configDraft, apiBaseUrl: value }) }),
              h('div', { style: { display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' } }, [
                h('button', { type: 'button', onClick: saveSettings, disabled: busy || !scope }, t.save),
                h('button', { type: 'button', onClick: startAuth, disabled: busy || !configDraft.appClientId.trim() }, t.authorize),
              ]),
            ...
            h('section', { style: { marginTop: 12 } }, plugins.length === 0
              ? h('p', null, t.empty)
              : plugins.map((plugin) => h('div', { key: plugin.moduleName + (plugin.presetId || ''), style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--dsw-alias-border-l1)' } }, [
                  h('div', null, [
                  ...
                  plugin.supported
                    ? h('button', { type: 'button', onClick: () => openReport(plugin), disabled: busy }, t.report)
      ...
      return h('li', { style: { listStyle: 'none', padding: 12, borderRadius: 10, border: '1px solid var(--dsw-alias-border-l1)' } },
        h('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' } }, h('strong', null, t.title), h('button', { type: 'button', onClick: () => setOpen(!open) }, open ? t.close : t.report)),
        body)
    }
    ...
    module.exports = { apply, inject: ['slots', 'locale', 'settingsScope', 'remote', 'remote.pluginInventory'] }
