window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      .ir-badge-ok { background: rgba(34, 197, 94, 0.15); color: #22c55e; }
      .ir-badge-warn { background: rgba(234, 179, 8, 0.15); color: #eab308; }
      .ir-badge-bad { background: rgba(239, 68, 68, 0.15); color: #ef4444; }
      ...
      .ir-tab-btn.active { color: var(--dsw-alias-label-primary); border-bottom-color: var(--dsw-alias-interactive-primary, #3b82f6); font-weight: 600; }
      ...
      .ir-btn-primary { background: var(--dsw-alias-interactive-primary, #3b82f6); color: #ffffff; border-color: transparent; }
      .ir-btn-primary:hover:not(:disabled) { background: #2563eb; }
      .ir-btn-danger { background: rgba(239, 68, 68, 0.15); color: #ef4444; border-color: transparent; }
      .ir-btn-danger:hover:not(:disabled) { background: rgba(239, 68, 68, 0.25); }
      ...
      .ir-alert-ok { padding: 8px 12px; border-radius: 6px; background: rgba(34, 197, 94, 0.15); color: #22c55e; font-size: 13px; }
      .ir-alert-warn { padding: 8px 12px; border-radius: 6px; background: rgba(234, 179, 8, 0.15); color: #eab308; font-size: 13px; }
      .ir-alert-bad { padding: 8px 12px; border-radius: 6px; background: rgba(239, 68, 68, 0.15); color: #ef4444; font-size: 13px; }
      ...
      .ir-dropzone-active { border-color: var(--dsw-alias-interactive-primary, #3b82f6); background: rgba(59, 130, 246, 0.05); }
      ...
      .ir-thumb-del { position: absolute; top: 2px; right: 2px; width: 18px; height: 18px; border-radius: 9999px; background: rgba(0,0,0,0.6); color: #fff; border: 0; font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
      .ir-thumb-meta { position: absolute; bottom: 2px; left: 2px; font-size: 9px; background: rgba(0,0,0,0.6); color: #fff; padding: 1px 3px; border-radius: 3px; }
    ...
    function useLocale(ctx) {
      const [locale, setLocale] = React.useState(() => {
        try { return ctx?.locale?.get?.() || 'en' } catch { return 'en' }
      })
      React.useEffect(() => {
        if (!ctx?.locale?.on) return
        try {
          return ctx.locale.on('change', (next) => setLocale(next || 'en'))
        } catch {}
      }, [ctx])
      return locale
    }
    ...
    function SettingsCardInner({ ctx }) {
                ...
                h('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap' } }, availableLabels.map((l) => {
                  ...
                  return h('button', {
                    ...
                    style: {
                      ...
                      borderRadius: 12,
                      fontSize: 11,
                      border: '1px solid #' + (l.color || 'cccccc'),
                      background: isSelected ? '#' + (l.color || '3b82f6') : 'transparent',
      ...
      const body = open ? h('div', { className: 'ir-page' }, [
        ...
        updateNotice ? h('div', {
          className: updateNotice.includes('fail') || updateNotice.includes('Error') ? 'ir-alert-bad' : 'ir-alert-ok',
