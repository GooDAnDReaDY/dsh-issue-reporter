window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      authHelp: 'Sign in with a GitHub OAuth App to search duplicates and create issues. Your token stays in DSH Credentials.',
      ...
      attachmentsHelp: 'Optional: add up to 5 PNG, JPG, GIF, or WebP screenshots. They are uploaded only after you confirm the issue.',
      removeAttachment: 'Remove',
      attachmentTooMany: 'Select up to 5 screenshots.',
      attachmentTooLarge: 'Each screenshot must be 8 MB or smaller; the total is limited to 20 MB.',
      attachmentReadError: 'Could not read screenshot',
    }
const zh = {
      title: '\u95ee\u9898\u62a5\u544a',
      ...
      authHelp: '\u4f7f\u7528 GitHub OAuth App \u767b\u5f55\u540e\u53ef\u641c\u7d22\u91cd\u590d\u9879\u5e76\u521b\u5efa issue\u3002\u4ee4\u724c\u4fdd\u5b58\u5728 DSH Credentials \u4e2d\u3002',
      signIn: '\u4f7f\u7528 GitHub \u767b\u5f55',
      signInAgain: '\u91cd\u65b0\u4f7f\u7528 GitHub \u767b\u5f55',
      signOut: '\u9000\u51fa\u767b\u5f55',
      signedOut: '\u5df2\u9000\u51fa GitHub',
      ...
      disabled: '\u5df2\u7981\u7528',
      attachments: '\u622a\u56fe',
      attachmentsHelp: '\u53ef\u9009\uff1a\u6dfb\u52a0\u6700\u591a 5 \u5f20 PNG\u3001JPG\u3001GIF \u6216 WebP \u622a\u56fe\u3002\u53ea\u6709\u5728\u786e\u8ba4 issue \u540e\u624d\u4f1a\u4e0a\u4f20\u3002',
      removeAttachment: '\u79fb\u9664',
      attachmentTooMany: '\u6700\u591a\u9009\u62e9 5 \u5f20\u622a\u56fe\u3002',
      attachmentTooLarge: '\u6bcf\u5f20\u622a\u56fe\u4e0d\u5f97\u8d85\u8fc7 8 MB\uff0c\u603b\u5927\u5c0f\u4e0d\u5f97\u8d85\u8fc7 20 MB\u3002',
      attachmentReadError: '\u65e0\u6cd5\u8bfb\u53d6\u622a\u56fe',
    }
    ...
    function readAttachment(file) {
      return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
          ...
          resolve({ name: file.name, mime: file.type, data, size: file.size })
        }
    ...
    function SettingsCard({ ctx }) {
      ...
      const onAttachmentChange = async (event) => {
        ...
        if (files.some((file) => file.size > MAX_ATTACHMENT_BYTES) || total > MAX_ATTACHMENT_TOTAL_BYTES) {
      ...
      const previewPanel = preview ? h('div', { style: { marginTop: 10, padding: 10, borderRadius: 8, background: 'var(--dsw-alias-bg-layer-2)' } }, [
        ...
        attachments.length ? h('p', null, t.attachments + ': ' + attachments.map((item) => item.name).join(', ')) : null,
      ...
      const editorPanel = editor ? h('section', { style: { marginTop: 14, paddingTop: 10, borderTop: '2px solid var(--dsw-alias-border-l1)' } }, [
        ...
        h('div', { style: { marginTop: 10 } }, [
          h('label', { style: { display: 'block' } }, [
            h('strong', null, t.attachments),
            h('input', { type: 'file', accept: 'image/png,image/jpeg,image/gif,image/webp', multiple: true, onChange: onAttachmentChange, style: { display: 'block', marginTop: 6 } }),
          ]),
          h('small', { style: { display: 'block', color: 'var(--dsw-alias-label-secondary)', marginTop: 4 } }, t.attachmentsHelp),
          attachments.length
            ? h('ul', { style: { margin: '8px 0 0', paddingLeft: 20 } }, attachments.map((file, index) => h('li', { key: file.name + '-' + index }, [
                h('span', null, file.name + ' (' + Math.max(1, Math.round(file.size / 1024)) + ' KB) '),
                h('button', { type: 'button', onClick: () => setAttachments(attachments.filter((_, current) => current !== index)) }, t.removeAttachment),
              ])))
            : null,
        ]),
      ...
      const renderPlugin = (plugin) => {
        ...
        const content = [
          h('div', { style: { flex: '1 1 auto', minWidth: 0, overflow: 'hidden' } }, [
            h('strong', { style: { display: 'block', overflowWrap: 'anywhere', wordBreak: 'break-word' } }, plugin.displayName),
          ...
          plugin.supported
            ...
            : h('span', { style: { flex: '0 0 auto', whiteSpace: 'nowrap' } }, plugin.enabled ? t.enabled : t.disabled),
        ]
            ...
            h('section', { style: { marginTop: 12, paddingTop: 8, borderTop: '1px solid var(--dsw-alias-border-l1)' } }, [
              ...
              status?.config?.tokenConfigured
                ? h('div', { style: { display: 'flex', gap: 8, flexWrap: 'wrap' } }, [
                    h('button', { type: 'button', onClick: startAuth, disabled: busy }, t.signInAgain),
                    h('button', { type: 'button', onClick: signOut, disabled: busy }, t.signOut),
                  ])
              ...
              auth?.signedOut ? h('p', null, t.signedOut) : null,
