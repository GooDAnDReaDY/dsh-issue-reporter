    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (llm && typeof llm.stream === 'function') {
          try {
            const defaultSelection = ctx.get ? ctx.get('agentDefaultModel')?.currentSelection?.() : null
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const sessionLog = ctx.get ? (ctx.get('session-log') || ctx.get('sessionLog')) : null
  ...
  if (ctx.tools && typeof ctx.tools.register === 'function') {
    ctx.tools.register({
      ...
      execute: async (args) => {
        ...
        const draft = composeIssueDraft({
          title: `[${(args.severity || 'bug').toUpperCase()}] ${args.title}`,
        ...
        if (!repository) {
          return {
            ok: false,
            error: `No public repository found for plugin ${args.plugin_name}`,
            draft,
          }
        }
