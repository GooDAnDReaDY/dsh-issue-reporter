...
В настройках GitHub App должен быть включён Device Flow. Временный код устройства и OAuth-токен запрашиваются через github.com, а api.github.com используется только для авторизованных GitHub API-запросов. Ошибка 404 Not Found при входе означает, что запущена старая сборка, отправляющая OAuth-запрос на REST API-домен.
