...
### 5. Автономный инструмент для агентов (`report_issue`)
Регистрирует инструмент в Cordis `ctx.tools`, позволяя автономным агентам и оркестраторам подготавливать или отправлять структурированные баг-репорты при сбоях установленных плагинов.

### 6. AI-оптимизация и генератор шагов воспроизведения
Интеграция с `ctx.llm` через `POST /dsh-issue-reporter/ai/optimize` позволяет в один клик превратить разрозненные заметки об ошибке в аккуратные шаги воспроизведения и понятный заголовок.

### 7. Журнал сессии и выбор сниппетов
Позволяет просматривать недавние логи сессии DSH через `GET /dsh-issue-reporter/logs` с автоматической десенсибилизацией секретов и прикреплять нужные строки к отчету.

### 8. Отслеживание статуса отправленных тикетов
Вкладка «Мои отчеты» хранит историю созданных обращений и выполняет пакетную проверку статусов (`POST /dsh-issue-reporter/issues/batch-status`), отображая актуальное состояние (open/closed) и количество комментариев.

### 9. Умные метки и автотегирование
Запрашивает доступные метки репозитория через `POST /dsh-issue-reporter/labels` и предлагает релевантные теги (`bug`, `ui`, `performance`, `auth`).

### 10. Автообновление плагина в один клик
Встроенный механизм обновления из npm-реестра: проверяет наличие свежей версии и позволяет обновить плагин прямо из карточки настроек.
